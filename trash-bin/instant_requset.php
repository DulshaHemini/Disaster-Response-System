...
$sql    = "SELECT * FROM Instant_Request ORDER BY req_id DESC";
...
<head>
    ...
    <title>Instant Requests · DRCS Admin</title>
...
<div class="admin-nav">
    <div class="nav-brand">
        ...
        <span class="admin-badge">� Instant Requests</span>
    </div>
        ...
        <table>
            ...
            <tbody>
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <?php
                            ...
                            $badgeClass = match(true) {
                                str_contains($statusNorm, 'pending')     => 'badge-pending',
                                str_contains($statusNorm, 'progress')    => 'badge-progress',
                                str_contains($statusNorm, 'resolved')
                                || str_contains($statusNorm, 'complete') => 'badge-resolved',
                                default                                  => 'badge-pending',
                            };
                        ?>
                        <tr>
                            <td><?php echo htmlspecialchars($row['req_id'],          ENT_QUOTES); ?></td>
                            <td><?php echo htmlspecialchars($row['user_id'] ?? '—',  ENT_QUOTES); ?></td>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="11" style="text-align:center; color:var(--muted);">No instant requests found</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
